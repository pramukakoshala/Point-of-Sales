...
//            }
            ...
            $printcmd = "java -classpath C:/xampp/htdocs/tto/application/views/dashboard/terminal/2/print.jar org.apache.pdfbox.PrintPDF -silentPrint -printerName HP-ENVY-4500 C:/xampp/htdocs/tto/doc.pdf";
            exec($printcmd);
            // include 'print_cmd.php';
            if (!isset($_GET["from"])) {
                if (intval($_GET["t"]) == 1) {
                    header("location:dashboard_ter?invoice=$finalcode&id=cash");
                } else if (intval($_GET["t"]) == 2) {
                    header("location:dashboard_ter1?invoice=$finalcode&id=cash");
                } else {
                    header("location:dashboard_ter2?invoice=$finalcode&id=cash");
                }
            } else {
                header("location:dashboard_trans");
            }
        }
    }
}
